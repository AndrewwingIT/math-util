package com.andrew.mathutil.Main;
...
public class Main {
    public static void main(String[] args) {
        testFactorialGivenRightArgumentOfReturnWell();
        //System.out.println("0! " + MathUtility.getFactorial(0));
        testFactorialGivenRightArgument5fReturnWell();
    }
    ...
    // STEPS/PROCEDURES:
    //  CALL getFactorial() method ưith valid arg: getFactorial(5)

    // EXPECTED RESULT:
    //          To have number 120 as the result of 5!;

    static void testFactorialGivenRightArgument5fReturnWell() {
    ...
    //          VÀ ĐI SO SÁNH VỚI EXPECTED Ở TRÊN ĐỀ LUẬN ĐÚNG SAI!!!!!

    //  MLEM VÌ 0! TRẢ VỀ 1 THẬT MÀ!

    // TA SẼ TEST HÀM CỦA TA VIẾT RA, GỌI LÀ LÀ UNIT TESTING - TEST NHỮNG ĐƠN VỊ
    // CODE CƠ BẢN NHẤT
    // TA CẦN TEST HÀM GETFACTORIAL() VỚI CÁC BỘ DATA ĐƯA VÀO - NHỮNG TEST CASE
    // TEST CASE LÀ GÌ: LÀ NHỮNG
    // BỘ DATA ĐƯA VÀO HÀM, APP, VÀ CÁCH THỨC XÀI
    ...
    //  CALL getFactorial() method ưith valid arg: getFactorial()
    ...
    static void testFactorialGivenRightArgumentOfReturnWell() {
        int n = 0;  // dự tính làm gì
        long expectedResult = 1; //0! hy vọng là 1
        long actualResult = MathUtility.getFactorial(0); //tính thử 0! bằng bao nhêu
